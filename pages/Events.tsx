...
const Events: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'events' | 'host' | 'ambassador'>('events');
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h1 className="text-6xl font-black italic uppercase mb-6 tracking-tighter">
            Community <span className="text-red-600">Action</span>
          </h1>
          <p className="text-neutral-400 max-w-2xl mx-auto text-lg font-light italic mb-8">
            Fitness is a team sport. Join our runs, workshops, and high-intensity meetups.
          </p>
          
          <div className="inline-flex bg-neutral-900 rounded-full p-1 border border-white/10 flex-wrap justify-center gap-2">
              <button 
                onClick={() => setActiveTab('events')}
                className={`px-6 py-2 rounded-full font-bold uppercase text-xs tracking-widest transition-all ${activeTab === 'events' ? 'bg-red-600 text-white' : 'text-neutral-400 hover:text-white'}`}
              >
                  Upcoming Events
              </button>
              <button 
                onClick={() => setActiveTab('host')}
                className={`px-6 py-2 rounded-full font-bold uppercase text-xs tracking-widest transition-all ${activeTab === 'host' ? 'bg-red-600 text-white' : 'text-neutral-400 hover:text-white'}`}
              >
                  Host an Event
              </button>
              <button 
                onClick={() => setActiveTab('ambassador')}
                className={`px-6 py-2 rounded-full font-bold uppercase text-xs tracking-widest transition-all ${activeTab === 'ambassador' ? 'bg-red-600 text-white' : 'text-neutral-400 hover:text-white'}`}
              >
                  Become Ambassador
              </button>
          </div>
        ...
        {activeTab === 'host' && (
            <div className="max-w-xl mx-auto bg-neutral-900 p-8 rounded-[40px] border border-white/5">
                <div className="text-center mb-8">
                    <h3 className="text-2xl font-bold mb-2">Host a Community Event</h3>
                    <p className="text-neutral-400 text-sm">Organize a run, workshop, or meetup in your city.</p>
                </div>
                <EventHostForm />
            </div>
        )}
        ...
        {activeTab === 'ambassador' && (
            <div className="max-w-xl mx-auto bg-neutral-900 p-8 rounded-[40px] border border-white/5">
                <div className="text-center mb-8">
                    ...
                    <p className="text-neutral-400 text-sm">Lead the movement. Represent FitMan Nation in your region.</p>
                </div>
                <AmbassadorForm />
            </div>
        )}
